...
document.addEventListener("DOMContentLoaded", function () {
    ...
    canvas.addEventListener("mousedown", function (event) {
        if (Modo === "linea") {
            ...
            if (x >= area1.startX && x <= area1.startX + area1.width &&
                y >= area1.startY && y <= area1.startY + area1.height) {
                var fillColor = hexToRgb(document.getElementById("relleno").value);
                var targetColor = getColorAtPixel(area1.startX, area1.startY);
                floodFill(area1.startX, area1.startY, targetColor, fillColor);
            }

            if (x >= area2.startX && x <= area2.startX + area2.width &&
                y >= area2.startY && y <= area2.startY + area2.height) {
                var fillColor = hexToRgb(document.getElementById("relleno").value);
                var targetColor = getColorAtPixel(area2.startX, area2.startY);
                floodFill(area2.startX, area2.startY, targetColor, fillColor);
            }
